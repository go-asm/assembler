...
//
//	func foo(fn func()) {
//		if github.com/go-asm/go/abi.FuncPCABIInternal(fn) == github.com/go-asm/go/abi.FuncPCABIInternal(Concrete) {
//			Concrete()
//		} else {
//			fn()
//		}
...
func maybeDevirtualizeFunctionCall(p *pgo.Profile, fn *ir.Func, call *ir.CallExpr) (ir.Node, *ir.Func, int64) {
	...
	// are generated by github.com/go-asm/go/cmd/compile/reflectdata.genhash.
...
func rewriteFunctionCall(call *ir.CallExpr, curfn, callee *ir.Func) ir.Node {
	...
	//
	// fnPC := github.com/go-asm/go/abi.FuncPCABIInternal(fn)
	// concretePC := github.com/go-asm/go/abi.FuncPCABIInternal(concrete)
	//
	// if fnPC == concretePC {
