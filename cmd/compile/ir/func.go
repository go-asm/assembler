...
type Inline struct {
	...
	// making inlining decisions). See github.com/go-asm/go/cmd/compile/inline/inlheur.
	Properties string
...
func IsFuncPCIntrinsic(n *CallExpr) bool {
	...
	return (fn.Name == "FuncPCABI0" || fn.Name == "FuncPCABIInternal") &&
		fn.Pkg.Path == "github.com/go-asm/go/abi"
}
...
// function as uintptr, as returned by github.com/go-asm/go/abi.FuncPC{ABI0,ABIInternal}.
//
// n should be a Node of an interface type, as is passed to
// github.com/go-asm/go/abi.FuncPC{ABI0,ABIInternal}.
//
...
func FuncPC(pos src.XPos, n Node, wantABI obj.ABI) Node {
	if !n.Type().IsInterface() {
		base.ErrorfAt(pos, 0, "github.com/go-asm/go/abi.FuncPC%s expects an interface value, got %v", wantABI, n.Type())
	}
	...
	if fn := IsIfaceOfFunc(n); fn != nil {
		...
		abi := fn.ABI
		if abi != wantABI {
			base.ErrorfAt(pos, 0, "github.com/go-asm/go/abi.FuncPC%s expects an %v function, %s is defined as %v", wantABI, wantABI, name.Sym().Name, abi)
		}
	...
	if wantABI != obj.ABIInternal {
		base.ErrorfAt(pos, 0, "github.com/go-asm/go/abi.FuncPC%s does not accept func expression, which is ABIInternal", wantABI)
	}
