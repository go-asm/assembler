...
import (
	...
	"github.com/go-asm/go/cmd/compile/bitvec"
	"github.com/go-asm/go/cmd/compile/compare"
	...
	"github.com/go-asm/go/cmd/compile/typebits"
...
func writeType(t *types.Type) *obj.LSym {
	...
	// +--------------------------------+                            - B
	// | github.com/go-asm/go/abi.UncommonType      |   dextratype
	...
	switch t.Kind() {
	case types.TARRAY:
		// github.com/go-asm/go/abi.ArrayType
		s1 := writeType(t.Elem())
	...
	case types.TSLICE:
		// github.com/go-asm/go/abi.SliceType
		s1 := writeType(t.Elem())
		c.Field("Elem").WritePtr(s1)

	case types.TCHAN:
		// github.com/go-asm/go/abi.ChanType
		s1 := writeType(t.Elem())
		c.Field("Elem").WritePtr(s1)
		c.Field("Dir").WriteInt(int64(t.ChanDir()))

	case types.TFUNC:
		// github.com/go-asm/go/abi.FuncType
	...
	case types.TINTER:
		// github.com/go-asm/go/abi.InterfaceType
		m := imethods(t)
		n := len(m)
		for _, a := range m {
			writeType(a.type_)
	...
	case types.TMAP:
		// github.com/go-asm/go/abi.MapType
		s1 := writeType(t.Key())
		s2 := writeType(t.Elem())
	...
	case types.TPTR:
		// github.com/go-asm/go/abi.PtrType
	...
	case types.TSTRUCT:
		// github.com/go-asm/go/abi.StructType
		fields := t.Fields()
