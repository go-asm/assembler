...
func genOp() {
	...
	fmt.Fprintln(w, "\"github.com/go-asm/go/cmd/obj\"")
	for _, a := range archs {
		if a.pkg != "" {
			fmt.Fprintf(w, "%q\n", a.pkg)
		}
	...
	for _, a := range archs {
		...
		for i, r := range a.regnames {
			num[r] = int8(i)
			pkg := a.pkg[len("github.com/go-asm/go/cmd/obj/"):]
			var objname string // name in github.com/go-asm/go/cmd/obj/$ARCH
			switch r {
			case "SB":
				// SB isn't a real register.  github.com/go-asm/go/cmd/obj expects 0 in this case.
				objname = "0"
			case "SP":
				objname = pkg + ".REGSP"
			case "g":
