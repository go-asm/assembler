...
func walkCall(n *ir.CallExpr, init *ir.Nodes) ir.Node {
	...
	if isFuncPCIntrinsic(n) {
		// For github.com/go-asm/go/abi.FuncPCABIxxx(fn), if fn is a defined function, rewrite
		...
		if isIfaceOfFunc(arg) {
			...
			abi := fn.Func.ABI
			if abi != wantABI {
				base.ErrorfAt(n.Pos(), 0, "github.com/go-asm/go/abi.%s expects an %v function, %s is defined as %v", name, wantABI, fn.Sym().Name, abi)
			}
		...
		if wantABI != obj.ABIInternal {
			base.ErrorfAt(n.Pos(), 0, "github.com/go-asm/go/abi.%s does not accept func expression, which is ABIInternal", name)
		}
