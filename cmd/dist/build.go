...
func findgoversion() string {
	...
	//
	// Note that we lightly parse github.com/go-asm/go/goversion/goversion.go to
	...
	goversionSource := readfile(pathf("%s/src/github.com/go-asm/go/goversion/goversion.go", goroot))
	m := regexp.MustCompile(`(?m)^const Version = (\d+)`).FindStringSubmatch(goversionSource)
	if m == nil {
		fatalf("github.com/go-asm/go/goversion/goversion.go does not contain 'const Version = ...'")
	}
...
// mustLinkExternal is a copy of github.com/go-asm/go/platform.MustLinkExternal,
...
var gentab = []struct {
	...
	{"github.com/go-asm/go/cmd/go/cfg", "zdefaultcc.go", mkzdefaultcc},
	{"runtime/github.com/go-asm/go/sys", "zversion.go", mkzversion},
...
func runInstall(pkg string, ch chan struct{}) {
	...
	ispkg := !strings.HasPrefix(pkg, "github.com/go-asm/go/cmd/") || strings.Contains(pkg, "/") || strings.Contains(pkg, "/vendor/")
...
// github.com/go-asm/go/cmd/go/imports/build.go.
...
func shouldbuild(file, pkg string) bool {
	...
	for _, p := range strings.Split(readfile(file), "\n") {
		...
		if code == "package dist" && pkg != "cmd/go" && pkg != "cmd/cgo" {
			return false
		}
...
func checkNotStale(env []string, goBinary string, targets ...string) {
	...
	if strings.Contains(out, "\tSTALE ") {
		...
		for _, target := range []string{"runtime/github.com/go-asm/go/sys", "cmd/dist", "cmd/link"} {
...
// "runtime/github.com/go-asm/go/*" packages.
//
// Keep in sync with github.com/go-asm/go/cmd/objabi/path.go:IsRuntimePackagePath.
