...
func (t *tester) maybeLogMetadata() error {
	...
	// could be simplified to directly use github.com/go-asm/go/sysinfo here.
	return t.dirCmd(filepath.Join(goroot, "src/github.com/go-asm/go/cmd/metadata"), gorootBinGo, []string{"run", "main.go"}).Run()
}
...
func (t *tester) registerTests() {
	...
	registerStdTestSpecially := map[string]bool{
		...
		"github.com/go-asm/go/cmd/testdir": true,
	}
	...
	if goos == "wasip1" {
		t.registerTest("wasip1 host tests",
			&goTest{
				variant:   "host",
				pkg:       "runtime/github.com/go-asm/go/wasitest",
		...
		for shard := 0; shard < nShards; shard++ {
			...
			t.registerTest("../test",
				&goTest{
					...
					pkg:         "github.com/go-asm/go/cmd/testdir",
...
// in github.com/go-asm/go/cmd/link/ld/config.go.
...
func (t *tester) registerCgoTests(heading string) {
	cgoTest := func(variant string, subdir, linkmode, buildmode string, opts ...registerTestOpt) *goTest {
		gt := &goTest{
			variant:   variant,
			pkg:       "github.com/go-asm/go/cmd/cgo/" + subdir,
			buildmode: buildmode,
		}
		var ldflags []string
		if linkmode != "auto" {
	...
	case os == "aix", os == "android", os == "dragonfly", os == "freebsd", os == "linux", os == "netbsd", os == "openbsd":
		...
		default:
			...
			if err != nil {
				...
				cmd := t.dirCmd("src/github.com/go-asm/go/cmd/cgo/test", cc, "-xc", "-o", "/dev/null", "-static", "-")
...
// github.com/go-asm/go/testenv.HasParallelism, which can't be used here
...
func (t *tester) registerRaceTests() {
	...
	if t.cgoEnabled {
		// Building github.com/go-asm/go/cmd/cgo/test takes a long time.
		// There are already cgo-enabled packages being tested with the race detector.
		// We shouldn't need to redo all of github.com/go-asm/go/cmd/cgo/test too.
		// The race buildler will take care of this.
		// t.registerTest(hdr, &goTest{variant: "race", race: true, env: []string{"GOTRACEBACK=2"}, pkg: "github.com/go-asm/go/cmd/cgo/test"})
	}
	if t.extLink() {
...
// github.com/go-asm/go/platform.RaceDetectorSupported, which can't be used here
// because cmd/dist can not import internal packages during bootstrap.
...
// github.com/go-asm/go/platform.BuildModeSupported, which can't be used here
// because cmd/dist can not import internal packages during bootstrap.
	...
	case "c-archive":
		...
		case "linux":
			...
			default:
				...
				// per ParseFlags in
				// github.com/go-asm/go/cmd/compile/base/flag.go.
