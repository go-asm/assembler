...
import (
	...
	"github.com/go-asm/go/cmd/go/base"
	"github.com/go-asm/go/cmd/go/cfg"
	"github.com/go-asm/go/cmd/go/fsys"
	"github.com/go-asm/go/cmd/go/gover"
	"github.com/go-asm/go/cmd/go/imports"
	"github.com/go-asm/go/cmd/go/modfetch"
	"github.com/go-asm/go/cmd/go/modindex"
	"github.com/go-asm/go/cmd/go/modinfo"
	"github.com/go-asm/go/cmd/go/modload"
	"github.com/go-asm/go/cmd/go/par"
	"github.com/go-asm/go/cmd/go/search"
	"github.com/go-asm/go/cmd/go/str"
	"github.com/go-asm/go/cmd/go/trace"
	"github.com/go-asm/go/cmd/go/vcs"
	"github.com/go-asm/go/cmd/pkgpattern"
...
func disallowInternal(ctx context.Context, srcDir string, importer *Package, importerPath string, p *Package, stk *ImportStack) *PackageError {
	...
	// The generated 'testmain' package is allowed to access testing/github.com/go-asm/go/...,
	...
	// The sort package depends on github.com/go-asm/go/reflectlite, but during bootstrap
...
func findInternal(path string) (index int, ok bool) {
	...
	case strings.Contains(path, "/github.com/go-asm/go/"):
		return strings.LastIndex(path, "/github.com/go-asm/go/") + 1, true
	case path == "internal", strings.HasPrefix(path, "github.com/go-asm/go/"):
		return 0, true
	}
	return 0, false
}
...
// The rules are the same as for /github.com/go-asm/go/ except that a path ending in /vendor
...
func (p *Package) load(ctx context.Context, opts PackageOpts, path string, stk *ImportStack, importPos []token.Position, bp *build.Package, err error) {
	...
	if useBindir {
		...
		if cfg.BuildBuildmode == "shared" && p.Internal.Build.PkgTargetRoot != "" {
			// TODO(matloob): This shouldn't be necessary, but the github.com/go-asm/go/cmd/cgo/testshared
...
// There is a copy of this function in github.com/go-asm/go/cmd/compile/gc/noder.go.
...
func SelectCoverPackages(roots []*Package, match []func(*Package) bool, op string) []*Package {
	...
	for _, p := range roots {
		...
		// and/or runtime/github.com/go-asm/go/atomic when using atomic coverage
		// mode. Atomic coverage mode uses sync/atomic, so we can't
		// also do coverage on it.
		if cfg.BuildCoverMode == "atomic" && p.Standard &&
			(p.ImportPath == "sync/atomic" || p.ImportPath == "runtime/github.com/go-asm/go/atomic") {
			continue
		}
		...
		// $GOROOT/src/github.com/go-asm/go/coverage/pkid.go dealing with
