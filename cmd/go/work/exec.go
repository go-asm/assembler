...
import (
	...
	"github.com/go-asm/go/cmd/buildid"
...
OverlayLoop:
	...
	// This is read by readGccgoArchive in github.com/go-asm/go/cmd/buildid/buildid.go.
...
func (b *Builder) gccld(a *Action, objdir, outfile string, flags []string, objs []string) error {
	...
	if len(out) > 0 {
		...
		// See also github.com/go-asm/go/cmd/link/ld's hostlink method.
		var save [][]byte
		var skipLines int
