...
import (
	...
	"github.com/go-asm/go/trace/v2"
	"github.com/go-asm/go/trace/v2/testtrace"
)
	...
	testTraceProg(t, "stacks.go", func(t *testing.T, tb, _ []byte, stress bool) {
		...
		if runtime.GOOS != "windows" && runtime.GOOS != "plan9" {
			want = append(want, []evDesc{
				{trace.EventStateTransition, "Goroutine Running->Waiting", []frame{
					{"github.com/go-asm/go/poll.(*FD).Accept", 0},
					{"net.(*netFD).accept", 0},
				...
				{trace.EventStateTransition, "Goroutine Running->Syscall", []frame{
					{"syscall.read", 0},
					{"syscall.Read", 0},
					{"github.com/go-asm/go/poll.ignoringEINTRIO", 0},
					{"github.com/go-asm/go/poll.(*FD).Read", 0},
					{"os.(*File).read", 0},
					{"os.(*File).Read", 0},
					{"main.main.func11", 0},
				}},
